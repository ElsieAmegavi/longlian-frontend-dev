...
export default function Details() {
	...
	return (
		...
		{/* Generator Details */}
<section className='w-full flex flex-col-reverse md:flex-row items-center justify-between mt-20 py-10 px-4 md:px-10'>
      ...
      <div>
        ...
        <div className='py-5'>
          <button
            ...
            type='submit'
            className='w-full sm:w-44 text-xl h-12 bg-orange-600 text-white rounded mx-auto uppercase tracking-wider font-semibold'
          >
            {t("Make Enquiry")}
          </button>
        </div>
      ...
      <div className=''>
        ...
        <div className='py-5'>
          <button
            ...
            type='submit'
            className='w-full sm:w-44 text-xl h-12 bg-white text-orange-600 border border-orange-600 rounded mx-auto uppercase tracking-wider font-semibold'
          >
            {t("Order Online")}
          </button>
        </div>
  ...
  <div className='w-full md:w-1/2 flex justify-center'>
    <img src={productList?.picture_url || ''} alt={productList?.model} className='max-w-full h-auto' />
  </div>
</section>

			{/* Technical Problems */}
			<section className='w-full flex flex-col items-center justify-center py-5 px-4'>
  <h3 className='font-semibold text-3xl md:text-5xl text-black text-center'>{t("Technical Problems")}</h3>

  <div className='w-full flex flex-col items-center my-5'>
    <div className='w-full md:w-10/12'>
      <div className='flex flex-col md:flex-row border-y h-auto md:h-16 w-full items-center justify-between px-4 md:px-96 py-2'>
        <p className='text-center md:text-left'>
          {productList?.model} {productList?.prime}
        </p>
        <p className='text-center md:text-right'>{productList?.voltage}</p>
      </div>
      <div className='flex flex-col md:flex-row border-b h-auto md:h-16 w-full items-center justify-between px-4 md:px-96 py-2'>
        <p className='text-center md:text-left'>{productList?.prime}</p>
        <p className='text-center md:text-right'>{productList?.voltage}</p>
      </div>
    </div>
    <div className='w-full md:w-10/12'>
      <div className='flex flex-col md:flex-row border-y h-auto md:h-16 w-full items-center justify-between px-4 md:px-96 py-2 opacity-25'>
        <p className='text-center md:text-left'>{productList?.voltage}</p>
        <p className='text-center md:text-right'>
          {productList?.model} {productList?.prime}
        </p>
      </div>
      <div className='flex flex-col md:flex-row border-b h-auto md:h-16 w-full items-center justify-between px-4 md:px-96 py-2 opacity-10'>
        <p className='text-center md:text-left'>{productList?.voltage}</p>
        <p className='text-center md:text-right'>{productList?.prime}</p>
      </div>
    </div>
  </div>
  ...
  <div className='py-5 w-full md:w-auto'>
    <button
      type='submit'
      className='w-full md:w-auto text-xl h-12 bg-white text-orange-600 border border-orange-600 rounded px-5 pt-2 pb-2 uppercase tracking-wider font-semibold flex flex-row items-center justify-center'
    >
  ...
  <div className='w-full flex justify-center'>
    <div className='w-full md:w-8/12 flex flex-col md:flex-row items-center'>
      ...
      <div className='w-full md:w-1/3 flex justify-center md:justify-end'>
        <Link
          to='/contactus'
          className='w-full md:w-48 p-5 text-lg h-20 bg-orange-600 text-white rounded uppercase tracking-wider font-semibold flex items-center justify-center'
        >
          {t("Contact Us")}{' '}
        </Link>
      </div>
